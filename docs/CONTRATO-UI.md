...
| La cámara sigue siendo cámara y no galería | Sin `capture` el navegador abre la galería y la evidencia deja de ser una toma del lugar. La excepción está en `ENTRADAS_SIN_CAPTURE`: `/poliza` adjunta documentación cargada antes del hecho, y su `accept` declara PDF, así que ahí `capture` impedía elegir el archivo. Tampoco lo lleva una entrada con `accept="application/pdf"` (el «Cargar PDF» de las tomas de papeles), por el mismo motivo |
